...
with open("src/adapters/opencode/index.ts", "r", encoding="utf-8") as f:
    code = f.read()

# 1. Inject injectModelPersona function
nl = chr(10)
# Use literal backslashes for TS \n
ts_nl = "\n"
...
inject_func += "    prompt = '<Role_Description>" + ts_nl + "' + description + '" + ts_nl + "</Role_Description>" + ts_nl + ts_nl + "' + prompt;" + nl
inject_func += "  }" + nl
inject_func += "  const modelLower = model ? model.toLowerCase() : '';" + nl
inject_func += "  if (modelLower.includes('gemini')) {" + nl
inject_func += "    prompt += '" + ts_nl + ts_nl + "<CRITICAL_MODEL_INSTRUCTION>" + ts_nl + "' +" + nl
inject_func += "              '## YOU MUST USE TOOLS FOR EVERY ACTION. THIS IS NOT OPTIONAL." + ts_nl + "' +" + nl
inject_func += "              '**YOUR FAILURE MODE**: You believe you can reason through file contents, task status, and verification without actually calling tools. You CANNOT. Your internal state about files you \'already know\' is UNRELIABLE." + ts_nl + "' +" + nl
inject_func += "              '1. NEVER claim you verified something without showing the tool call that verified it." + ts_nl + "' +" + nl
inject_func += "              '2. NEVER reason about what a changed file \'probably looks like.\' Call Read on it." + ts_nl + "' +" + nl
...
inject_func += "    prompt += '" + ts_nl + ts_nl + "<CRITICAL_MODEL_INSTRUCTION>" + ts_nl + "' +" + nl
inject_func += "              'Strictly adhere to the required output formats and tool schemas. Do NOT output markdown code blocks unless requested. Do NOT hallucinate parameters. Focus only on the requested task." + ts_nl + "' +" + nl
inject_func += "              '</CRITICAL_MODEL_INSTRUCTION>';" + nl
inject_func += "  }" + nl
inject_func += "  if (agentName === 'chief') {" + nl
inject_func += "    prompt += '" + ts_nl + ts_nl + "<ROLE_ENFORCEMENT>" + ts_nl + "' +" + nl
inject_func += "              'CRITICAL RULE: YOU MUST NEVER WRITE CODE, EXECUTE COMMANDS, OR DO THE WORK YOURSELF." + ts_nl + "' +" + nl
inject_func += "              'You are Atlas - Master Orchestrator. Role: Conductor, not musician. General, not soldier." + ts_nl + "' +" + nl
inject_func += "              'You DELEGATE, COORDINATE, and VERIFY. Your ONLY job is to break down the request, create a plan using todowrite, and delegate EVERY single implementation step to subagents using \'chief_task\'." + ts_nl + "' +" + nl
inject_func += "              'When subagents return, you MUST verify their work. Remember: Subagents lie, always verify using read or lsp tools." + ts_nl + "' +" + nl
...
inject_func += "    prompt += '" + ts_nl + ts_nl + "<ROLE_ENFORCEMENT>" + ts_nl + "' +" + nl
inject_func += "              'You are an IMPLEMENTER. You DO NOT delegate tasks. You use your available tools to complete the work assigned to you directly and completely. You NEVER use \'chief_task\'." + ts_nl + "' +" + nl
inject_func += "              '</ROLE_ENFORCEMENT>';" + nl
inject_func += "  }" + nl
...
print("Patch Final Python Script Finished!")
