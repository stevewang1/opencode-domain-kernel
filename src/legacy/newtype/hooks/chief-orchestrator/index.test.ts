...
import { createChiefOrchestratorHook } from "./index"
import {
  writeBoulderState,
  clearBoulderState,
  readBoulderState,
...
describe("chief-orchestrator hook", () => {
  const TEST_DIR = join(tmpdir(), "chief-orchestrator-test-" + Date.now())
  ...
  function createMockPluginInput(overrides?: { promptMock?: ReturnType<typeof mock> }) {
    ...
    } as unknown as Parameters<typeof createChiefOrchestratorHook>[0] & { _promptMock: ReturnType<typeof mock> }
  }
  ...
  describe("tool.execute.after handler", () => {
    test("should ignore non-chief_task tools", async () => {
      // #given - hook and non-chief_task tool
      const hook = createChiefOrchestratorHook(createMockPluginInput())
      const output = {
        title: "Test Tool",
    ...
    test("should not transform when caller is not chief", async () => {
    ...
    test("should append standalone verification when no boulder state but caller is orchestrator", async () => {
      ...
      setupMessageStorage(sessionID, "chief")
    ...
    test("should transform output when caller is chief with boulder state", async () => {
      // #given - chief caller with boulder state
    ...
    describe("Write/Edit tool direct work reminder", () => {
      ...
      beforeEach(() => {
        setupMessageStorage(ORCHESTRATOR_SESSION, "chief")
      })
      ...
      test("should append delegation reminder when orchestrator writes outside .chief/", async () => {
        // #given
        const hook = createChiefOrchestratorHook(createMockPluginInput())
        const output = {
          title: "Write",
      ...
      test("should NOT append reminder when orchestrator writes inside .chief/", async () => {
        // #given
        const hook = createChiefOrchestratorHook(createMockPluginInput())
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.chief/plans/work-plan.md" },
        }
      ...
      test("should NOT append reminder when non-orchestrator writes outside .chief/", async () => {
        // #given
  ...
  describe("session.idle handler (boulder continuation)", () => {
    ...
    beforeEach(() => {
      ...
      setupMessageStorage(MAIN_SESSION_ID, "chief")
    })
    ...
    test("should inject continuation when boulder has incomplete tasks", async () => {
      ...
      const hook = createChiefOrchestratorHook(mockInput)
    ...
    test("should not inject when last agent is not chief", async () => {
      // #given - boulder state with incomplete plan, but last agent is NOT chief
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      // #then - should NOT call prompt because agent is not chief
      expect(mockInput._promptMock).not.toHaveBeenCalled()
    })
